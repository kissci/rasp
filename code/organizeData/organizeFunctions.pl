			...
			if (-e $file) {
				...
				while (<FILEIN>) {
					if (/^([^;]+);([^;]+);(.*)$/) {
						...
						my $acttemp = $2;
						my $actpress = $3;
						if ($acttime =~ m/^[0-9]+$/ && $acttime >= $startdate && $acttime <= $enddate) {
							if ($sens eq 'bmp180') {
								my $startinterval = $startdate+(int(($acttime-$startdate)/$interval))*$interval;
								...
								push($data{$startinterval}->{$sens}->{'temp' }->{'data'}, $acttemp);
								push($data{$startinterval}->{$sens}->{'press'}->{'data'}, $actpress);
							}
						}
					}
				}
